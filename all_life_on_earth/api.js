 ...
 * - BUGFIX: Fixed Firebase Storage permission error by enforcing .jpg extension for thumbnails
 * and passing correct Content-Type metadata during upload.
 */
...
export async function uploadMedia(file, userId) {
    ...
    if (file.type.startsWith('video/')) {
        ...
        // Pass content type metadata for the video
        const metadata = { contentType: file.type };
        
        const snap = await uploadBytes(videoRef, file, metadata);
    ...
    // FIX: Explicitly append .jpg so filename matches the mime type generated by createThumbnail
    const thumbPath = `users/${userId}/specimens/${timestamp}-${file.name}_thumb.jpg`;
    ...
    // FIX: Add metadata for strict security rules
    const metadataOriginal = { contentType: file.type };
    const metadataThumb = { contentType: 'image/jpeg' };

    const [originalSnap, thumbSnap] = await Promise.all([
        uploadBytes(originalRef, file, metadataOriginal),
        uploadBytes(thumbRef, thumbBlob, metadataThumb)
    ]);
