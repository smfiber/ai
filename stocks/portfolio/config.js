...
export const CONSTANTS = {
    ...
    DB_DOC_ID_USER_10Q: 'user_mda_summary',
...
**Instruction:** Create 3-4 bullet points summarizing the most critical, data-supported findings from your detailed analysis. Each point must directly and accurately compare a key metric (e.g., Valuation, Profitability) for {companyName} against the peer median. You must explicitly state if {companyName} is higher, lower, or similar, and what that implies, without contradicting the detailed analysis below.
...
- **Valuation for Unprofitable Companies:** If a company's P/E or EV/EBITDA is 'N/M' or negative, you MUST state that this is because the company is unprofitable. In this specific case, do not use P/E for comparison and instead focus on the P/S ratio.
...
- Does this data suggest better or worse profitability and efficiency than its peers? For negative metrics, explicitly state whether the company's performance is superior (less negative) or inferior (more negative).
...
Role: You are a quantitative investment analyst AI. Your task is to generate a concise, forward-looking valuation analysis for {companyName} ({tickerSymbol}). Your analysis must be objective and based *exclusively* on the provided data.

Data Instructions:
- You will be provided a JSON object containing analyst estimates and historical valuation multiples.
- Your entire analysis MUST be derived from this data. Do not infer or use outside knowledge. If a data point is "N/A", you must state that clearly.
...
Generate the following forward-looking analysis report:

# Forward-Looking Analysis: {companyName} ({tickerSymbol})

## 1. Analyst Growth Expectations
- **EPS Growth:** Analysts expect EPS to grow by **{estimated_eps_growth}** over the next year.
- **Revenue Growth:** Analysts expect revenue to grow by **{estimated_revenue_growth}** over the next year.
