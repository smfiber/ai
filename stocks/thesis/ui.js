    ...
    analysisModal.addEventListener('click', async (e) => {
        ...
        if (copyReportBtn) {
            ...
            if (symbol && reportType) {
                handleCopyReportRequest(symbol, reportType, copyReportBtn);
            }
            return;
        }
        ...
        // --- NEW LISTENER FOR SAVE FILING CHECK-IN ANSWERS ---
        if (target.id === 'save-filing-checkin-answers-button') {
            handleSaveFilingCheckinAnswers(symbol);
            return;
        }
        // --- END NEW LISTENER ---
        ...
        // --- NEW LISTENER FOR FINAL THESIS SAVE BUTTON ---
        if (target.id === 'save-final-thesis-answers-button') {
            handleSaveFinalThesisAnswers(symbol);
            return;
        }
        // --- END NEW LISTENER ---
