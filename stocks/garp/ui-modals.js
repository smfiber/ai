...
export async function openRawDataViewer(ticker) {
    ...
    try {
        ...
        if (peerDocSnap.exists()) {
            ...
            const peerContentContainer = peerAnalysisContainer.querySelector('#peer-analysis-content-container');
            const peerInput = peerAnalysisContainer.querySelector('#manual-peer-input');
            
            renderPeerComparisonTable(peerContentContainer, ticker, metrics, peerData);
            if (peerInput && peerData.peers) {
                peerInput.value = peerData.peers.join(', ');
            }
        }
