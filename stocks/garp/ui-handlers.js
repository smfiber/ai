...
export function renderCandidacyAnalysis(container, reportContent, prompt, diligenceQuestions = []) {
    ...
    // Find the diligence questions section in the raw markdown
    const startToken = '## Actionable Diligence Questions';
    const endToken = '6. Critical Guidelines & Constraints:';
    const startIndex = rawMarkdown.indexOf(startToken);
    const endIndex = rawMarkdown.indexOf(endToken, startIndex);
    
    let questionsSectionHtml = '';

    if (startIndex !== -1 && endIndex !== -1) {
        const questionsMarkdown = rawMarkdown.substring(startIndex, endIndex);
        const questionsHtml = marked.parse(questionsMarkdown);
        
        // Use a DOM parser to find and replace the content
        ...
        doc.querySelectorAll('li').forEach(li => {
            ...
            if (strongTags.length === 2) {
                const humanQuestionEl = strongTags[0].nextSibling;
                const aiQueryEl = strongTags[1].nextSibling;
                
                if (humanQuestionEl && aiQueryEl) {
                    const humanQuestionText = humanQuestionEl.textContent.trim();
                    const aiQueryText = aiQueryEl.textContent.trim().replace(/^:/, '').replace(/^"|"$/g, '').trim();
                    ...
                    clickableDiv.innerHTML = `<p class="font-semibold text-sm text-indigo-800">${sanitizeText(humanQuestionText)}</p><span class="text-xs text-indigo-600 font-medium">Click to investigate this question</span>`;
        ...
        questionsSectionHtml = `<h4 class="text-lg font-bold text-gray-800 mb-2 mt-4 border-t pt-4">Actionable Diligence Questions</h4>${doc.body.innerHTML}`;
    }

    const finalHtml = accordionHtml + reportHtml + questionsSectionHtml;
