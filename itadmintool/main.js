...
// --- App Version ---
const APP_VERSION = "1.6.0"; // [REFACTORED] The 'Full Detailed Guide' generation process to be more robust, preventing missing sections by generating and assembling parts independently. [ADDED] Timestamps to Google Drive filenames.
...
async function generateFullDetailedGuide(button) {
    ...
    try {
        ...
        detailedContentEl.innerHTML = getLoaderHTML('Step 1/4: Generating guide components...');

        const [
            firstDraftResult,
            securityModuleResult,
            automationResourcesResult,
            helpfulResourcesResult
        ] = await Promise.allSettled([
            callGeminiAPI(getMasterGuidePrompt('fullGuide', context), false, "Generate Full Guide (Draft)"),
        ...
        detailedContentEl.innerHTML = getLoaderHTML('Step 2/4: Verifying and assembling...');

        const firstDraftMarkdown = firstDraftResult.status === 'fulfilled' ? (firstDraftResult.value || '') : '';
        const allGeneratedSections = {};

        const sectionParts = firstDraftMarkdown.split(/(?=###\s+\d+\.)/g);
        sectionParts.forEach(part => {
            const headerMatch = part.match(/###\s+(\d+)\.\s+[\w\s&]+/);
            if (headerMatch) {
                const sectionNumber = headerMatch[1].trim();
                allGeneratedSections[sectionNumber] = part;
            }
        });

        allGeneratedSections['8'] = automationResourcesResult.status === 'fulfilled' ? `### 8. Automation Techniques\n\n${automationResourcesResult.value}` : `### 8. Automation Techniques\n\n*Error generating automation resources: ${automationResourcesResult.reason?.message || 'Unknown error'}*`;
        allGeneratedSections['9'] = securityModuleResult.status === 'fulfilled' ? securityModuleResult.value : `### 9. Security Considerations\n\n*Error generating security module: ${securityModuleResult.reason?.message || 'Unknown error'}*`;
        allGeneratedSections['12'] = helpfulResourcesResult.status === 'fulfilled' ? `### 12. Helpful Resources\n\n${helpfulResourcesResult.value}` : `### 12. Helpful Resources\n\n*Error generating helpful resources: ${helpfulResourcesResult.reason?.message || 'Unknown error'}*`;

        const requiredSectionNumbers = ['5', '6', '7', '8', '9', '10', '11', '12'];
        const missingSectionNumbers = requiredSectionNumbers.filter(num => !allGeneratedSections[num]);

        if (missingSectionNumbers.length > 0) {
            detailedContentEl.innerHTML = getLoaderHTML(`Step 3/4: Repairing ${missingSectionNumbers.length} missing section(s)...`);
            
            const repairPromises = missingSectionNumbers.map(num => {
                const header = `### ${num}. ${getMasterGuidePrompt('fullGuide', context).match(new RegExp(`###\\s+${num}\\.\\s+([\\w\\s&]+)`))?.[1] || `Section ${num}`}`;
                const repairPrompt = `You are completing an IT guide on "${coreTopic}". The guide is missing a section. Your task is to generate ONLY the content for the following missing section, ensuring it is complete and detailed. Do NOT include the header itself in your response, only the content that should follow it.\n\nMissing Section Header: "${header}"\n\nExisting Guide Context:\n${blueprintMarkdown}`;
                
                return callGeminiAPI(repairPrompt, false, `Repair Section ${num}`)
                    .then(content => {
                        allGeneratedSections[num] = `${header}\n\n${content || `*Content generation failed for section ${num}.*`}`;
                    }).catch(error => {
                        console.error(`Repair failed for section ${num}:`, error);
                        allGeneratedSections[num] = `${header}\n\n*Content generation failed for section ${num}.*`;
                    });
            });
            await Promise.all(repairPromises);
        }

        detailedContentEl.innerHTML = getLoaderHTML('Step 4/4: Assembling final document...');

        let finalCompleteGuideMarkdown = blueprintMarkdown;
        for (const num of requiredSectionNumbers) {
            finalCompleteGuideMarkdown += `\n\n${allGeneratedSections[num] || `### ${num}. Section Missing\n\n*Content could not be generated for this section.*`}`;
        }
...
function renderAccordionFromMarkdown(markdownText, containerElement) {
    ...
    nodes.forEach(node => {
        if (!firstHeaderFound && node.tagName !== 'H3' && node.tagName !== 'H4') {
